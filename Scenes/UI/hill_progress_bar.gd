...
func update_progress(value: float):
	...
	if is_player_alive[1]: $Player2Icon.position = Vector2(icon_x_start + icon_x_offset, size.y - icon_y_offset - size.y * value / 100)
	if is_player_alive[2]: $Player3Icon.position = Vector2(icon_x_start + 2 * icon_x_offset, size.y - icon_y_offset - size.y * value / 100)
	if is_player_alive[3]: $Player4Icon.position = Vector2(icon_x_start + 3 * icon_x_offset, size.y - icon_y_offset - size.y * value / 100)
